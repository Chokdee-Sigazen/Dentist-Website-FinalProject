...
export default function FormPanel() {
  ...
  useEffect(() => {
    const fetchDentistData = async () => {
      try {
        ...
        if (topic) {
          setFilteredDentists(
            allDentist.data.filter((x: Dentist) =>
              x.areaOfExpertise.includes(topic)
            )
          );
        } else {
          setFilteredDentists(allDentist.data);
        }
      } catch (error) {
  ...
  const MakeAppt = () => {
    if (topic && dentistSelected && dateAppt && timeAppt) {
      const combinedDateTime = dateAppt
        .hour(timeAppt.hour())
        .minute(timeAppt.minute());
  ...
  return (
    <div className="bg-[#F1C2C4] rounded-md px-10 py-10 justify-center space-y-10 font-bold text-[#895355] text-[14pt] drop-shadow-sm mx-5 my-5">
      ...
      <button
        name="Book Vaccine"
        className="w-[100%] block rounded-md bg-[#61C0BF] 
                hover:bg-cyan-700 px-3 py-2 text-white shadow-md"
        onClick={MakeAppt}
      >
        Submit
      </button>

...
export default async function PastApptPanel() {
  ...
  if (session?.user) {
    ...
    const appointmentData: ApptInfoProps[] = appointments.data.filter((appointment: ApptInfoProps) => {
      return appointment.finish === false && (role === "user" ? appointment.user === id : true);
    });
